...
class DiaconnG8Service : DaggerService() {
        ...
        if (sp.getBoolean(R.string.key_diaconn_g8_cloudsend, true)) {
            ...
            try {
                ...
                if(response?.body()?.ok == true) {
                    ...
                    val (platformStart, platformEnd, platformLoopSize) = getLogLoopCount(platformLastNo.toInt(), platformWrappingCount, pumpLastNum, pumpWrappingCount)
    ...
    private fun getLogLoopCount(lastLogNum: Int, wrappingCount: Int, pumpLastNum: Int, pumpWrappingCount: Int): Triple<Int, Int, Int> {
        ...
        val end: Int // log sync end number

        if (pumpWrappingCount * 10000 + pumpLastNum - lastLogNum > 10000) {
            start = pumpLastNum
            end = 10000
        } else if (pumpWrappingCount > wrappingCount && lastLogNum < 9999 && wrappingCount != -1) {
            start = (lastLogNum + 1)
            end = 10000
    ...
    fun updateBasalsInPump(profile: Profile): Boolean {
        ...
        sendMessage(requestReqPacket4, 500)
